...
RSpec.describe 'Api::V1::Users', type: :request do
  describe 'GET api/v1/users/:user_id/authored_courses' do
    context 'with valid parameters' do
      it 'it returns a list of authored_courses' do
        user = create(:user)
        create_list(:course, 3, author: user)
        get api_v1_user_authored_courses_path(user),
    ...
    context 'unauthorized user' do
      it 'it returns an unauthorized error' do
        user = create(:user)
        get api_v1_user_authored_courses_path(user), headers: {
